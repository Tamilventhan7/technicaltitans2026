...
exports.getSimulationSpeed = getSimulationSpeed;
exports.startSimulation = startSimulation;
exports.stopSimulation = stopSimulation;
exports.manualInjectIncident = manualInjectIncident;
...
async function tick() {
    ...
    const vehicles = await (0, db_1.getVehicles)();
    const drivers = await (0, db_1.getDrivers)();
    for (const trip of activeTrips) {
        ...
        if (vehicle.type === 'Sprinter Van')
            targetSpeed = 95;
        else if (vehicle.type === 'Medium Cargo')
            targetSpeed = 85;
        // Slow down in heavy weather/traffic
        if (config.weatherSeverity === 'storm' || config.weatherSeverity === 'snow')
            targetSpeed -= 30;
        else if (config.weatherSeverity === 'rain')
            targetSpeed -= 15;
        if (config.trafficLevel === 'heavy')
            targetSpeed -= 20;
        else if (config.trafficLevel === 'jammed')
            targetSpeed -= 45;
        // Apply speed limits if delayed or in-transit
        if (trip.status === 'delayed')
            targetSpeed = Math.min(targetSpeed, 40); // slow or broken down
        // Move speed closer to target
        if (speed < targetSpeed)
            speed = Math.min(targetSpeed, speed + 10);
        else if (speed > targetSpeed)
            speed = Math.max(targetSpeed, speed - 15);
        ...
        if (driver.activeHoursToday > 11) {
            ...
            const existing = (await (0, db_1.getAlerts)()).find(a => a.id === fatigueAlertId);
            if (!existing) {
                await triggerAlert({
                    id: fatigueAlertId,
                    tripId: trip.id,
        ...
        if (vehicle.currentFuel < (vehicle.fuelCapacity * 0.15)) {
            ...
            const existing = (await (0, db_1.getAlerts)()).find(a => a.id === fuelAlertId && !a.resolved);
            if (!existing) {
                await triggerAlert({
                    id: fuelAlertId,
        ...
        if (newIdx >= trip.route.length - 1) {
            ...
            // Badges
            if (driver.gamification.points > 12000)
                driver.gamification.tier = 'Diamond';
            else if (driver.gamification.points > 8000)
                driver.gamification.tier = 'Gold';
            else if (driver.gamification.points > 4000)
                driver.gamification.tier = 'Silver';
            await triggerAlert({
                id: `AL-DEL-${trip.id}`,
                tripId: trip.id,
        ...
        // Save states
        await (0, db_1.saveVehicle)(vehicle);
        await (0, db_1.saveDriver)(driver);
        await (0, db_1.saveTrip)(trip);
    }
    // Socket notification
    if (ioInstance) {
        // Generate summarized KPIs for this tick
        const allTrips = await (0, db_1.getTrips)();
        const liveAlerts = (await (0, db_1.getAlerts)()).filter(a => !a.resolved);
...
async function findActiveTripAlert(tripId, category) {
    const alerts = await (0, db_1.getAlerts)();
...
async function triggerAlert(alert) {
    await (0, db_1.saveAlert)(alert);
    if (ioInstance) {
        ioInstance.emit('alert-triggered', alert);
    }
}
// Injects an incident manually
async function manualInjectIncident(tripId, category) {
    const trips = await (0, db_1.getTrips)();
...
async function injectRandomAnomaly(trip, vehicle, driver, location, forcedCategory) {
