...
router.get('/health', async (_req, res) => {
    try {
        ...
        const fleetHealthAvg = Math.round(vehicles.reduce((s, v) => s + (v.healthScore || 85), 0) / Math.max(vehicles.length, 1));
        const driverSafetyAvg = Math.round(drivers.reduce((s, d) => s + (d.safetyScore || 80), 0) / Math.max(drivers.length, 1));
        res.json({
            success: true,
            data: {
                fleetHealthAvg,
...
router.get('/predict-maintenance', async (_req, res) => {
    try {
        const vehicles = await (0, db_1.getVehicles)();
        const predictions = vehicles.map((v) => {
            ...
            if (daysFactor <= 3)
                riskLevel = 'critical';
            else if (daysFactor <= 7)
                riskLevel = 'high';
            else if (daysFactor <= 15)
                riskLevel = 'medium';
    ...
    try {
        ...
        if (activeAlerts.length > 0) {
            recs.push({
                id: 'alert-summary',
                type: 'risk',
                priority: activeAlerts.filter((a) => a.severity === 'critical').length > 0 ? 'critical' : 'high',
                title: `${activeAlerts.length} Unresolved Fleet Alerts`,
                msg: `${activeAlerts.filter((a) => a.severity === 'critical').length} critical and ${activeAlerts.filter((a) => a.severity === 'warning').length} warning alerts need immediate attention.`,
                confidence: 100,
...
router.get('/carbon-report', async (_req, res) => {
    try {
        const vehicles = await (0, db_1.getVehicles)();
        const trips = await (0, db_1.getTrips)();
        const completedTrips = trips.filter((t) => t.status === 'delivered');
        const totalFuelEstimate = completedTrips.reduce((s, t) => s + (t.fuelUsed || 80), 0);
        ...
        res.json({
            ...
            data: {
                ...
                weeklyData,
                vehicleEmissions: vehicles.slice(0, 5).map((v, i) => ({
                    vehicleId: v.id,
...
router.get('/financial-forecast', async (_req, res) => {
    try {
        const trips = await (0, db_1.getTrips)();
        const completedTrips = trips.filter((t) => t.status === 'delivered');
        const totalRevenue = completedTrips.reduce((s, t) => s + (t.financials?.revenue || 0), 0);
        const totalProfit = completedTrips.reduce((s, t) => s + (t.financials?.profit || 0), 0);
        const forecast = [
...
router.get('/document-expiry', async (_req, res) => {
    try {
        const vehicles = await (0, db_1.getVehicles)();
        const drivers = await (0, db_1.getDrivers)();
